...
$titulo = "Livraria - Menu de Gêneros";
...
if(isset($_GET['p']) && is_numeric($_GET['p'])){
    ...
    $q = "SELECT COUNT(id) FROM genero WHERE descricao like '%$where%'";
...
switch ($ordem){
    case 'id' : $order_by = 'id';
    break;
    case 'descricao' : $order_by = 'descricao';
    break;
    default:
    $order_by= 'id';
    $ordem= 'id';
...
$q = "SELECT id, descricao FROM genero WHERE descricao like '%$where%' ORDER BY $order_by LIMIT $inicio, $exiba";
...
if(mysqli_num_rows($r) > 0){
    $saida = '<div class="table-responsive col-md-12">
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th width="10%"><a href="genero_menu.php?ordem=id">Código</a></th>
                            <th width="70%"><a href="genero_menu.php?ordem=desc">Descrição</a></th>
                            <th>Ações</th>
                        </tr>
                    </thead>
                    <tbody>';
                    ...
                    while($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
                        ...
                        <td>' . $row['descricao'] . '
                        <td>
                            <a href="genero_alt.php?id='. $row['id'] . '"class="btn btn-sm btn-warning"> Alterar</a>
                            <a href="genero_exc.php?id='. $row['id'] . '"class="btn btn-sm btn-danger"> Excluir</a>
                        </td>
                        </tr>';
                    }
...
if($pagina > 1) {
    ...
    //botão anterior
    if($pagina_correta != 1) {
        $pag .= '<li class="page-item"><a class="page-link" href="genero_menu.php?s=' . ($inicio - $exiba) . '&p=' . $pagina . '&ordem=' . $ordem . '">Anterior</a></li>';
    } else {
    ...
    for($i = 1; $i <= $pagina; $i++) {
        if($i != $pagina_correta){
            $pag .= '<li class="page-item"><a class="page-link" href="genero_menu.php?s=' . ($exiba * ($i - 1)) . '&p=' . $pagina . '&ordem=' . $ordem . '">' . $i . '</a></li>';
        } else {
    ...
    if($pagina_correta != $pagina){
        $pag .= '<li class="page-item"><a class="page-link" href="genero_menu.php?s=' . ($inicio + $exiba) . '&p=' . $pagina . '&ordem=' . $ordem  . '">Próximo</a></li>';
    } else {
  ...
  <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
      ...
      <div class="row mt-3 ">
        ...
        <div class="col-md-3 text-right">
            <a href="genero_cad.php" class="btn btn-primary">
             Inserir gênero
            </a>
        </div>
      </div>
